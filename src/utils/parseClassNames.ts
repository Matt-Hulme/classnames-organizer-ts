    ...
    JSXAttribute(path) {
      ...
      if (path.node.value && path.node.value.type === "StringLiteral") {
        ...
        if (expression.type === "StringLiteral") {
          ...
          (expression.callee as Identifier).name === "classNames"
        ) {
          ...
          expression.quasis.forEach((quasi) => {
            ...
            const sortedClassNames = sortClassNames(classNames);
            replacements.push({
              start: quasi.start + 1, // exclude the starting backtick
              end: quasi.end - 1, // exclude the ending backtick
              value: `${sortedClassNames}`, // no need to enclose in backticks here
            });
          });
        }
      }
